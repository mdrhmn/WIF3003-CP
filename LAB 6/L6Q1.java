...
/*
 * Write a program to find the integer in the range of 1 and 111111 that has
 * the largest number of divisors. The program will divide the task into several
 ...
 * compared to get the final result. A sample output is given below for the
 * range of integers between 1 and 2500.
 */
public class L6Q1 {

    private final static int MAX = 111111;

    /**
     * A class to represent the result from one task. The result consists of the
     * maximum number of divisors in the range of integers assigned to that task,
     * and the integer in the range that gave the maximum number of divisors.
     */
     ...
     * the constructor. The task is executed when the call() method is called. At
     * the end of the call() method, a Result object is created to represent the
     * results from this task, and the result object is returned as the value of
     * call().
     */
     ...
     * ExecutorService. The Futures that are returned when the tasks are submitted
     * are placed into an ArrayList. The results from those Futures are combined to
    ...
    private static void countDivisorsWithExecutor(int numberOfThreads) {
        ...
        /*
         * Create the tasks and add them to the executor. Each task consists of a range
         * of 1000 integers, so the number of tasks is (MAX+999)/1000. (The "+999" gives
         * the correct number of tasks when MAX is not an exact multiple of 1000. The
         * last task in that case will consist of the last (MAX%1000)) ints.
         */
         ...
         * that are stored in the ArrayList. Get the results and combine them to produce
         * the final output. Note that each call to res.get() blocks, if necessary,
    ...
    /**
     * The main() routine just gets the number of threads from the user and calls
     * countDivisorsWithThreads() to do the actual work.
     */
    ...
    /**
     * Finds the number of divisors of the integer N. Note that this method does the
     * counting in a stupid way, since it tests every integer in the range 1 to N to
     * see whether it evenly divides N.
     */
